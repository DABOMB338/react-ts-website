...
function TextSprite({ position = [0, 0, 0], text = '', sectionIndex = 0, isActive = false, headerHeight = 0, onSwipeLeft, onSwipeRight, onScrollableChange, onContentEndY }: {
  ...
  const canvasMeasurements = useMemo(() => {
    ...
    const section = SECTION_CONTENT[text] || { lines: [], isHeader: [], lineHeightFactor: 0.02 };
    const lines = section.lines;
    const isHeader = section.isHeader;
    const lineHeight = Math.round(section.lineHeightFactor * height / dpr) + extraSpacing;
